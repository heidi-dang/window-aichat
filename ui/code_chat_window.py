...
class TextLineNumbers(tk.Canvas):
    ...
    def redraw(self, *args):
        ...
        while True:
            ...
            self.create_text(
                2, y, anchor="nw", text=linenum, fill="#606366", font=("Consolas", 9)
            )
...
class CodeChatWindow(tk.Toplevel):
    def __init__(self, parent, chat_client):
        ...
        self.diff_hunks = (
            []
        )  # Stores (orig_start_line, orig_end_line, ai_start_line, ai_end_line)
    ...
    def setup_ui(self):
        ...
        tk.Button(
            toolbar, text="📂 Open Folder", command=self.open_folder, bg="#ecf0f1"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            toolbar, text="📄 Open File", command=self.open_file, bg="#ecf0f1"
        ).pack(side=tk.LEFT, padx=5)

        self.diff_view_btn = tk.Button(
            toolbar, text="Show Diff", command=self.toggle_diff_view, bg="#ecf0f1"
        )
        self.diff_view_btn.pack(side=tk.LEFT, padx=5)

        self.prev_change_btn = tk.Button(
            toolbar,
            text="< Prev Change",
            command=self._go_to_prev_change,
            bg="#ecf0f1",
            state=tk.DISABLED,
        )
        self.prev_change_btn.pack(side=tk.LEFT, padx=2)
        self.next_change_btn = tk.Button(
            toolbar,
            text="Next Change >",
            command=self._go_to_next_change,
            bg="#ecf0f1",
            state=tk.DISABLED,
        )
        self.next_change_btn.pack(side=tk.LEFT, padx=2)

        tk.Button(toolbar, text="🔍 Find", command=self.find_text, bg="#ecf0f1").pack(
            side=tk.LEFT, padx=5
        )
        ...
        self.main_paned = tk.PanedWindow(
            self, orient=tk.HORIZONTAL, sashwidth=4, bg="#bdc3c7"
        )
        ...
        tk.Label(
            left_frame, text="Explorer / Changes", font=("Segoe UI", 9, "bold")
        ...
        self.code_paned = tk.PanedWindow(
            self.main_paned, orient=tk.HORIZONTAL, sashwidth=4, bg="#bdc3c7"
        )
        ...
        tk.Label(chat_frame, text="Chat History", font=("Segoe UI", 9, "bold")).pack(
            anchor="w", padx=5, pady=2
        )
        self.chat_history = scrolledtext.ScrolledText(
            chat_frame, wrap=tk.WORD, font=("Segoe UI", 9), state=tk.DISABLED
        )
        self.chat_history.pack(fill=tk.BOTH, expand=True, padx=5, pady=2)

        # Configure chat tags
        self.chat_history.tag_config(
            "user", foreground="#2980b9", font=("Segoe UI", 9, "bold")
        )
        self.chat_history.tag_config(
            "ai", foreground="#27ae60", font=("Segoe UI", 9, "bold")
        )
        self.chat_history.tag_config(
            "system", foreground="#7f8c8d", font=("Segoe UI", 9, "italic")
        )
        ...
        self.apply_btn = tk.Button(
            btn_frame,
            text="Apply Changes",
            command=self.apply_current_change,
            bg="#3498db",
            fg="white",
            font=("Segoe UI", 9),
        )
        ...
        self.revert_btn = tk.Button(
            btn_frame,
            text="Revert Changes",
            command=self.revert_current_file,
            bg="#e74c3c",
            fg="white",
            font=("Segoe UI", 9),
        )
    ...
    def setup_highlight_tags(self):
        ...
        for text_widget in [self.orig_text, self.ai_text]:
            ...
            text_widget.tag_configure(
                "Token.Name.Variable.Instance", foreground="#000000"
            )
    ...
    def refresh_file_tree(self, single_file=None):
        ...
        if single_file:
            self.file_tree.insert(
                "", "end", single_file, text=os.path.basename(single_file), open=True
            )
            return
        ...
        for root, dirs, files in os.walk(self.current_folder):
            ...
            for f in files:
                ...
                # Use full path as ID
                display_text = (
                    f"{f} ({os.path.relpath(full_path, self.current_folder)})"
                )
                tags = []

                self.file_tree.insert(
                    "", "end", full_path, text=display_text, tags=tuple(tags)
                )
    ...
    def apply_highlighting(self, text_widget, content, language):
        ...
        if language == "python":
            ...
            apply_regex(
                r"\b(def|class|return|if|else|elif|while|for|in|import|from|try|except|with|as|pass|break|continue|lambda|yield|async|await|global|nonlocal|assert|del|raise)\b",
                "token_keyword",
            )
            ...
            apply_regex(r'"[^"\n]*"|\'[^\'\n]*\'', "token_string")
            apply_regex(
                r"\b(function|return|if|else|for|while|do|switch|case|break|continue|var|let|const|class|new|this|import|export|public|private|protected|static|void|int|float|double|char|bool)\b",
                "token_keyword",
            )
            apply_regex(r"\b\d+\b", "token_number")
    ...
    def toggle_diff_view(self):
        if not self.selected_file or self.selected_file not in self.file_changes:
            messagebox.showinfo(
                "Info",
                "No AI-generated changes available for this file to create a diff.",
                parent=self,
            )
            return
        ...
        if self.is_diff_view:
            ...
            self.apply_highlighting(
                self.ai_text, new_content, self.detect_language(self.selected_file)
            )
    ...
    def display_side_by_side_diff(self, original_content, new_content):
        # Configure tags
        self.orig_text.tag_config(
            "diff_removed", background="#ffcccc", foreground="black"
        )
        self.orig_text.tag_config(
            "diff_changed", background="#fff5cc", foreground="black"
        )
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "replace":
                ...
                self.diff_hunks.append(
                    (i1, i2, j1, j1)
                )  # j1, j1 for no change in AI side
            elif tag == "insert":
                self.ai_text.tag_add("diff_added", f"{j1+1}.0", f"{j2}.end")
                self.diff_hunks.append(
                    (i1, i1, j1, j2)
                )  # i1, i1 for no change in original side
    ...
    def _flash_hunk(self, text_widget, start_index, end_index):
        original_bg = text_widget.tag_cget(
            "flash_tag", "background"
        ) or text_widget.cget("bg")
        ...
        self.after(
            300,
            lambda: text_widget.tag_configure(
                "flash_tag", background=original_bg, foreground="black"
            ),
        )
        self.after(
            600,
            lambda: text_widget.tag_configure(
                "flash_tag", background="yellow", foreground="black"
            ),
        )
        self.after(
            900,
    ...
    def _go_to_prev_change(self):
        ...
        self.current_hunk_index = (
            self.current_hunk_index - 1 + len(self.diff_hunks)
        ) % len(self.diff_hunks)
        self._go_to_hunk(self.current_hunk_index)
        self._update_navigation_buttons()
    ...
    def find_text(self, event=None):
        ...
        search_str = simpledialog.askstring(
            "Find", "Enter text to search:", parent=self
        )
        if not search_str:
            return
...
"""
        ...
        threading.Thread(
            target=self._process_ai_request, args=(full_prompt,), daemon=True
        ).start()
        ...
        if matches:
            ...
            for filepath, content in matches:
                ...
                if not os.path.isabs(filepath) and self.current_folder:
                    ...
                    filepath = (
                        self.selected_file
                    )  # Fallback to current file if path is ambiguous
    ...
    def apply_current_change(self):
        ...
        messagebox.showinfo(
            "Applied",
            "Changes staged in memory. Click 'Save All Changes' to write to disk.",
        )
    ...
    def save_all_changes(self):
        ...
        if messagebox.askyesno(
            "Save All", f"Write changes to {len(self.file_changes)} files?"
        ):
            try:
